	...
	try {
		if (use === 'verification') {
			...
			mailOptions.html = emailLayout({
				...
				footer:
					'Este código expira en 30 minutos. Si no solicitaste este registro, ignora este correo.',
			})
			...
			mailOptions.html = emailLayout({
				...
				action: `<div style="background:#00284d; color:#efd9af; display:inline-block; padding:16px 32px; border-radius:8px; font-size:2rem; font-weight:bold; letter-spacing:6px; border: 2px solid #045389;">${code}</div>`,
				footer:
					'Este código expira en 15 minutos. Si no solicitaste este acceso, ignora este correo.',
			})
		} else {
			throw new Error(
				"Uso de correo inválido: debe ser 'verification' o 'authentication'"
			)
		}
